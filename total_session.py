...
class FlowEntry:
    ...
    def print_total_session(self):
        session_count = 0
        for i in self.flow_list:
            if self.flow_list[i]["session"] == S_EST:
                session_count = session_count + 1
            elif self.flow_list[i]["session"] == S_END:
                session_count = session_count + 1
            #end_if
            if self.flow_list[i]["count"] >= 0:
                session_count = session_count + self.flow_list[i]["count"]
        #end_for
        print session_count
    #end_def
